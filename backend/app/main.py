...
def export_short_boat_analysis_pdf(boat_id: int, start: date, end: date, is_captain: bool = False , db: Session = Depends(get_db)):
    data = crud.get_short_boat_analysis_data(db, boat_id, start_date=start, end_date=end)
    if not data:
        raise HTTPException(status_code=404, detail="Δεν βρέθηκαν δεδομένα")
    
    pdf_buffer = pdf_utils.generate_short_boat_analysis_pdf(data, is_captain)
